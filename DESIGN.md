...
**2026-09-08 parser handoff:** current deterministic parsing scores Prago
LAS_all **70.0%**, UAS_all **76.5%**, UPOS **89.5%**, and Cairo LAS_all
**81.9%**, UAS_all **85.9%**, UPOS **91.3%** on the frozen 131/20-sentence
fixtures — **both are development-informed regression fixtures, not held-out
generalization tests** (Cairo had been mislabeled "held-out" in places; its
errors have in fact informed development). Coverage is 99.9% / 100%.

A 2026-09-08 read-only audit found the September cycle had also shipped an
unguarded crash: an apposition rule (0f995ba) could attach a dependency
cycle, crashing `parse()` on ~2.5% of store sentences and 1/500 of the
`rebaseline_500` QA questions, undetected because the merge gate itself
could not run (`pytest -m contract`/`-m accuracy` failed to collect — a
module-level `pandas` import with no installed pandas). Both are **fixed**
(commits `1533468c`, `e3972228`): 0/3000 crashes on the same store sample,
both test tiers collect and pass, and the parser floors in
`tests/test_parser_ud_accuracy.py` are ratcheted to these current numbers
(they had drifted 7+ points below HEAD — a real 25-edge Prago regression
would have passed silently). `klareco#905` (the retriever swallowing schema
errors on the always-runs hot path) landed in the same pass. A second,
unrelated crash class remains open (klareco#929).

**Focus, decided 2026-09-08: the parser itself, not store consumption.**
Milestone #35 is closed; its P0 candidate-generation/decoder work is
demoted to research (milestone #39 — gated on #920's reviewed gold, which
has 0/200 reviewed sentences) since #914's own edge-recall gate is unmet
(76.45%/75.84% vs an 80%/85% target) and #926's headlined "+2 Prago edges"
changed 5 of 2,709 tokens in 2 sentences — not distinguishable from noise.
Active parser work is milestone **#38** (clause spine #922, PP/argument
candidates #918) and milestone **#37, "Parser: structured extensible AST
output"** — whether the parser's own output can host a rule-based
annotation layer, not just human/gold ingestion (klareco#935). Milestone
**#40, "Store consumption (parked)"**, holds the wiring-into-retrieval work
(formerly #33/#35's non-parser issues) explicitly on hold. Evidence for all
of the above is in `data/perf/parser_research/` and
`data/perf/bench_history.jsonl`.

The production store has **not** been rebuilt: 21 oversized rows require
source repair, and the independent 200-sentence annotation pilot remains
unreviewed. Historical store-consistency and QA claims below do not
...
**Current state (generated 2026-09-07 15:04 UTC)**
...
| Ontology | ✅ 12,798 nodes / 237,739 edges (ALIASO 224,527, HAVAS_SUPERKLASON 7,883, SINONIMO 2,864, HAVAS_ENTECAN_TIPON 2,337, APARTENAS_AL_VERBA_KLASO 128) |
...
| `ontology_nodes` / `ontology_edges` | **empty**, ontology absent | **LOADED and CONSUMED.** 12,798 nodes / **237,739** edges (the 13,212 figure recorded here omitted `ALIASO`'s 224,527, written by a separate loader). `DuckDBRetriever` loads the `ALIASO` bridge default-on (#872). |
| `verb_klaso` column | 0% populated | **Does not exist on `sentences`** and never has. It exists on `clauses` at 82.6% — but holds **POS/transitivity codes**, not semantic classes (`subst` 2.4M, `tr` 2.2M, `ntr` 878K). Real class membership is `ontology_edges.APARTENAS_AL_VERBA_KLASO`, 8 classes / 128 roots. See #907. |
...
`load_ontology.py` — but note the reason recorded here was wrong: the `(de, al,
rel)` schema claim is obsolete (it now creates the correct `(rel, radiko,
class_id)`, fixed at the source). The real reason is that it `DROP TABLE`s and
rebuilds from `onto.roots` only, destroying all **224,527 `ALIASO` edges**,
which come from a separate loader (#910). Thinness is real: 82% of nodes have `klaso=NULL`, the
...
`klareco/`) and, as of the 2026-07-14 rebuild, **complete in data** — the
ontology population step has been ported and run.

**3. The problem has moved from MISSING to UNREAD (2026-07-20).** With the
rebuild verified (#835/#836/#837/#838 all closed on measurement), a full trace
of the production path found the components are individually sound and *not
connected*: `clauses` (6.95M rows) and `dependency_arcs` (68.9M rows) have zero
production readers, the reranker's entity-type gate reads a column whose
vocabulary has zero overlap with what it tests for, and the stored AST blob is
lossy on expand. This is now the central problem, tracked as **#901** and
...
ProperNounRerankStage     proper-noun specificity boost (#877) — shipped, in-memory
RerankStage               STUB — appended but always skipped (no model registered)
  ↓
ExtractAndGenerateStage   ExtractiveAnswerGenerator over top-k passages
BiographyFormatStage      optional (off by default) — biography / definition (#775)
  ↓
FormatOutputStage         final text + citations
```

**`ASTAwareRerankStage` is NOT in the pipeline.** It is not imported by
`factory.py`; it was demoted 2026-07-18 (#895) after a rebaseline_210 A/B showed
MRR 0.3619 → 0.3446. It remains reachable from tests and
`scripts/eval/multi_reranker_bench.py`. Since it is the only component written
against the wide shredded schema, most of that schema currently serves nothing
in production (#904).

**What production actually reads from the store** (traced 2026-07-20): five
columns — `sentences(sid, subj_vortspeco, obj_radiko, text, ast_json)` — plus
`ontology_edges(rel='ALIASO')` loaded once at retriever construction. Every
other stage is pure in-memory work over the expanded AST. Two consequences worth
knowing:

- **The retriever swallows schema errors on the hot path.** A renamed column
  yields empty retrieval and `_NO_ANSWER` with no exception, so the #884
  `stage_failed` flag never fires. Meanwhile `preflight` validates
  `subj_radiko`, `aliaj_json` and `clauses.verb_klaso` — none of which the
  default pipeline reads — and does not validate the columns it does (#905).
- **Citations render with an empty title.** `article_title` is 100% populated
  (#803) but the retriever never selects it, so every `CitationRecord.doc_title`
  is `''` (#912).
...
⚠️ The table that stood here was measured 2026-07-12 against a store that no
longer exists. It listed `verb_klaso` and `verb_negated` as columns of
`sentences` — **neither has ever existed there** — and its row count and file
size were both wrong. Corrected below, measured 2026-07-20 over the live
4,624,110 rows (full scan, not a sample).

**Six tables, 47.3 GB.** `sentences` 4,624,110 · `clauses` 6,954,531 ·
`dependency_arcs` 68,887,052 · `entity_facts` 1,006,992 · `ontology_edges`
237,739 · `ontology_nodes` 12,798.

| `sentences` column | NULL% | Distinct | Note |
|---|---:|---:|---|
| `ast_json` | 0.0 | ~4.27M | the AST blob — **compact form; lossy on expand** (see below) |
| `aliaj_json` | 0.0 | ~3.90M | modifiers (loko / tempo / numeral bearing) |
| `text` | 0.0 | ~3.75M | |
| `subj_radiko` / `subj_vortspeco` | 28.6 | ~260,613 / 6 | |
| `verb_radiko` | 26.7 | ~27,497 | |
| `verb_tempo` | 29.9 | 3 | |
| `obj_radiko` | 74.2 | ~26,269 | |
| `subj_propranoma_kat` | 82.8 | 3 | **structural** categories only (see below) |
| `success_rate` | 0.0 | ~3,530 | real variance since #805 (was constant 0.0) |
| provenance (`source_name` … `quality`) | 0.0–33.3 | — | #803 |
| `subj_kazo` / `obj_kazo` | 28.6 / 74.2 | **1 / 1** | **zero information** — #908 |

Four data-quality facts worth internalizing:

- **`propra_nomo` over-triggering is largely fixed.** Proper-noun subjects are
  now **16.6%** of the store, down from 42% — the `REDIRECT` (42K) / `The` (29K)
  / `Ĝia` (29K) artifacts are gone (#821, #823 closed 2026-07-20).
- **`subj_propranoma_kat` speaks a different vocabulary than the reranker, and
  the overlap is exactly zero.** It carries the parser's *structural* categories
  — the only values across 4.6M rows are `propranomo` (722,594),
  `propranomo_esperantigita` (45,542), `neologismo` (27,150). The reranker gates
  on the ontology's *semantic* categories (`persono`, `loko`). **`persono` never
  appears in the column.** The gate has never fired once. The join exists —
  `ontology_edges.HAVAS_ENTECAN_TIPON` has `persono` (377 roots), `loko` (706) —
  it is simply never applied at index time (#906).
- **`clauses.verb_klaso` does not hold verb classes.** It holds POS/transitivity
  codes from the root lexicon: `subst` 2,407,104 · `tr` 2,230,211 · `ntr`
  877,611. **34.6% of rows in the column named for verbs are tagged noun**
  (#907).
- **Four columns are 100% populated and carry no signal**: `subj_kazo`/`obj_kazo`
  on both tables are single-valued and fully derivable from radiko-presence — a
  subject is nominative *by definition of the slot*. Case was captured in the one
  position where it is tautological and dropped everywhere it would discriminate
  (#908).

### The store holds three representations, and production reads the weakest

`parse()` returns three parallel views of every sentence, sharing token dicts by
reference:

| View | Shape | Status |
|---|---|---|
| `vortoj` | full UD dependency tree (`id`/`kapo`/`rolo`, 21 relations, all morphology) | **"the source of truth"**, per `compact_ast`'s own docstring |
| `propozicioj` | flat list of clause frames | authoritative for clause structure |
| `subjekto`/`verbo`/`objekto` | one flat frame | **legacy** |

Three findings from a 2026-07-20 trace, each now an issue under #901:

1. **The store shreds the legacy frame, and that frame mixes clauses.** It is
   documented as describing "the MAIN clause"; it does not. For *"La viro kiu
...
3. **The rich tables are dark.** `clauses` (6.95M) and `dependency_arcs` (68.9M)
   ...
   `ontology_edges(rel='ALIASO')` (#904).

Measured cost of the shape mismatch: **261,372 sentences** have a NULL flat
`subj_radiko` while a clause *does* carry a subject, and **2,263,488 clause
subjects (40.9%)** differ from the flat column and are reachable only via
`clauses`. 31.8% of sentences are multi-clause. For scale, the pending #807
reparse recovers ~182k subjects — the schema shape hides an order of magnitude
more.

**The store is therefore not under-informative. It is mis-projected**, and the
information needed to fix it is already on disk.
...
| *same `verb_klaso`* generalization (0.6 weight) | **DEAD** — no such column on `sentences`; `clauses.verb_klaso` holds POS tags (#907) |
| negation agreement | **DEAD** — no `verb_negated` column on `sentences` at all (being added, #807) |
| answer-type gating on `expected_kats` = `persono` | **DEAD** — store vocabulary and gate vocabulary have **zero overlap** (#906) |
...
**Updated 2026-07-20, and the diagnosis is now sharper than "empty columns".**
Two of these three are not sparse — they are *absent* or *mismatched*:

- `verb_klaso` and `verb_negated` were never columns of `sentences`. The
  2026-07-12 table that said "0.0%" and "1.1%" described a store that no longer
  exists. `verb_negated` is being added by the #807 reparse (9.4% negated,
  measured on a smoke build); `verb_klaso` exists on `clauses` but holds
  POS/transitivity, not semantic classes.
- The entity-type gate cannot fire *in principle*, not merely rarely: the column
  holds `{propranomo, propranomo_esperantigita, neologismo}` and the gate tests
  for `{persono, personaj_pronomoj}`. Disjoint sets.

And the reranker that reads them is no longer in the pipeline (#895). So the
honest statement is stronger than "running on nothing": **the smart reranker was
written against a schema the store never had, measured as worse than BM25, and
demoted.** Reviving it means fixing the substrate first (#906, #907, #904) — not
...
**None of these adds a capability.** They build the foundation that makes
capability work measurable — the thing this project has been missing since May.

| # | Milestone | What it delivers | Status |
|---|---|---|---|
| **[#16](https://github.com/marctjones/klareco/milestone/16)** | **Corpus & Index Integrity** | The data is honest: no redirect pollution, provenance preserved, parser artifacts restored | mostly **done** — #802/#803/#818/#821/#823/#777 closed 2026-07-20; what remains is missing *source* data (#804 proper-noun dicts, #810 supplemental corpora, #806 ReVo raw) |
| **[#19](https://github.com/marctjones/klareco/milestone/19)** | **Rebuild** | The fixed parser's output in the store, verified | ✅ **CLOSED 2026-07-20** — ran 2026-07-14, every check in #838 passes |
| **[#40](https://github.com/marctjones/klareco/milestone/40)** | **Store consumption (parked)** | The rich AST reaches retrieval — clause tree wired in, vocabularies reconciled, hot path fails loudly | **PARKED 2026-09-08** — sound plan, explicitly on hold while parser is the focus; was #33 |
| **[#17](https://github.com/marctjones/klareco/milestone/17)** | **Test Coverage** | We can tell when the system is unfit to test — tier gating, stage tests | open, parallel |
| **[#41](https://github.com/marctjones/klareco/milestone/41)** | **Measurement ruler** | The ruler that can actually *see* a reranker win, plus parser gold | **parked with #40** — was #20/#21/#22/#23 |
| **[#14](https://github.com/marctjones/klareco/milestone/14)** | **Stable Base** | The merge gate in force; unmeasured features finally measured | needs #41 |
| **[#36](https://github.com/marctjones/klareco/milestone/36)** | **Pre-parser work** | Crash bug + merge gate fixed before parser coding resumes | ✅ **mostly done 2026-09-08** — #927/#928/#905 closed; #929/#932 remain |
| **[#37](https://github.com/marctjones/klareco/milestone/37)** | **Parser: structured extensible AST output** | The parser's own output contract can host further annotations, not just human/gold ones | **ACTIVE, current focus** — #935 + children |
| **[#38](https://github.com/marctjones/klareco/milestone/38)** | **Parser accuracy — active** | Grammar work that doesn't need a reviewed gold set | **ACTIVE** — #918, #922, #863, #819/#820 |
| **[#39](https://github.com/marctjones/klareco/milestone/39)** | **Parser accuracy research (deferred)** | Candidate-recall/decoder work | **parked** — gated on #920's reviewed gold (0/200) |

### The dependency that governs everything — RESTATED AGAIN 2026-09-08

The July chain below (**#33 → #23 → #14**) was the plan until a 2026-09-08
audit found two things: the September parser cycle had shipped a crash
regression the (non-running) merge gate couldn't catch, and the store-33
critical path had sat completely untouched since 2026-07-20 (0 commits
referenced #901-#905 across 41 September commits) while parser-accuracy work
absorbed the effort instead. Rather than treat that as pure drift, the user
made an explicit decision: **focus is the parser itself, not store
consumption, until further notice.**

That is now the chain: fix what's broken (done — #927 crash, #928 gate,
#905 loud failure, all closed 2026-09-08), then the parser's own output
contract (milestone #37: does `klareco/ast_annotations.py`'s envelope
actually host a rule-based producer, not just human/gold ingestion — #935),
then continued grammar work that doesn't need a reviewed gold set (milestone
#38: #918, #922, #863, #819/#820). Milestone #40, "Store consumption
(parked)", holds everything below — #807/#839 (the one-rebuild epic),
#901-#913/#916 (Phase 0.5's substrate work), the Deep Band retrieval
residue (#868/#879/#880) — untouched but not lost. The July reasoning for
*why* the store matters still holds; it just isn't what's being worked on
right now. Milestone #35's decoder/candidate-recall track is similarly
parked as research (milestone #39) pending #920's reviewed gold (0/200
today).

**The July restatement, preserved for context (superseded, not deleted):**

The old chain read **#16 → #15 → #14**, on the premise that "the parser is
currently degraded (no proper-noun dictionary), so it tags every
sentence-initial capitalized word as `propra_nomo`". **That premise is now
false**, and keeping it would misdirect the next several weeks:

- The parser is repaired and **regression-guarded** against the UD gold
  treebanks (`pytest -m accuracy`, #900). `propra_nomo` subjects are down from
  42% to **16.6%** of the store (#821 closed).
- The corpus is honest: 0 redirect stubs, 0 markup, 0 English (#823 closed),
  provenance 100% populated (#803 closed), ontology loaded and consumed (#777
  closed).
- The rebuild ran and verified (#835/#836/#837/#838 all closed on measurement).
- The gold ruler exists at n=500 (`rebaseline_500`) with a paired-bootstrap CI
  harness, and has already converted a borderline result into a gate pass
  (#877).

**The new chain is #33 → #23 → #14: consume the store, then measure what
consuming it bought.**

The binding constraint has moved from *"the data is dishonest"* to *"the honest
data is unread"*. `clauses` and `dependency_arcs` hold 76M rows that no
production code queries; the reranker's discriminating inputs are absent or
vocabulary-mismatched. Fixing the substrate is a prerequisite for any reranker
work having a chance — but note the second half of the constraint, unchanged
since #736: **a discriminating test set still does not exist**, so wiring the
clause tree in cannot yet be *proven* to help. Run #33 and the
reranker-discrimination stratum (#23: #849/#850) in parallel, or #33 lands
unmeasured exactly as the 25 capabilities of 2026-05-26 did.

### Start here — UPDATED 2026-09-08

1. **Done** — #927 (crash guard), #928 (merge gate collects and runs again),
   #905 (retriever raises on schema drift instead of degrading silently), all
   closed 2026-09-08. Parser floors in `tests/test_parser_ud_accuracy.py`
   ratcheted to HEAD (were 7+ points stale). #902 also closed — the lossless
   v2 envelope answer already shipped in code (`klareco/ast_storage.py`).
2. **#935** — the open question for milestone #37: prove (or find the gap in)
   a rule-based `annotation_layers` producer, not just human/gold ingestion.
3. **#918, #922** (milestone #38) — clause spine and PP/argument candidates:
   real grammar work, no dependency on a reviewed gold set.
4. **#929** (milestone #36, "Pre-parser work") — a second, unrelated
   dependency-cycle class (2/3000, different cause from #927) — small, but
   worth closing before it's forgotten.
5. Everything under "The dependency that governs everything", below, is
   **parked, not abandoned** — #849/#850 (reranker-discrimination stratum,
   milestone #41) and #811/#784 remain valid and cheap whenever store-
   consumption work resumes.

**The July "Start here", preserved for context (superseded, not deleted):**

1. **#901 → #905, #902** — *make the live path honest, then decide what the store
   should hold.* #905 is cheap and is a correctness bug on the one path that
   always runs (schema drift currently yields `_NO_ANSWER` with no flag). #902
   decides whether `ast_json` keeps the lossy compact form or stores `vortoj` as
   the single source of truth — that decision constrains the schema, so it comes
   before any rebuild.
2. **#903 → #904** — *settle the shape.* One design conversation: is the
   queryable unit the sentence or the clause? The measured cost of the current
   answer is 261,372 sentences whose subject is invisible to the column
   production filters on.
3. **#807** — *one rebuild, not five, and not yet.* The reparse is built,
   rehearsed and ready (`scripts/pipeline/reparse_store_807.sh`), but its
   justification (#871) moved **zero** UD accuracy metrics and touches ~4% of
   the corpus. Running it into the *current* schema banks a small win and then
   reproduces every misalignment above. Carry it in the same pass as the aligned
   schema.
4. **#849/#850** — *the ruler for all of the above.* Without the
   reranker-discrimination stratum, none of #33 can clear the merge gate.

**#811** (tier gating) and **#784** (adopt the merge gate) remain valid and
parallel — both are cheap, and #811 is precisely what stops us scoring rerankers
against a column that does not exist, which is how nine rerankers came to look
"equivalent".
