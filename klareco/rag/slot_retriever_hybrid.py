"""
Hybrid FAISS + Mmap retriever combining best of both worlds.

Three-stage retrieval pipeline:
1. FAISS pre-filtering: 4.2M docs → 500 candidates (fast semantic search)
2. Mmap slot reranking: 500 docs → slot-aware scoring (high accuracy)
3. Final ranking: Combined scores → top-k results

Expected performance: 90% recall, ~3.5ms latency
- Matches MemoryMappedSlotRetriever recall (90%)
- Approaches FAISSSlotRetriever speed (5.1ms)

Memory optimizations:
- P1: FAISS memory mapping (disk-backed)
...
logger.info(f"Hybrid retriever FAISS thread count: {physical_cores} (physical cores)")


class HybridFAISSMmapRetriever:
    """Hybrid retriever combining FAISS pre-filtering with mmap slot reranking."""
    ...
    def _load_indexes(self):
        """Load FAISS index, mmap arrays, and build offset index."""
        logger.info(f"Loading hybrid FAISS+mmap indexes from {self.index_path}")
        ...
        logger.info(f"  Hybrid retriever ready: {len(self.doc_offsets):,} documents")

    def _load_faiss_index(self):
        """Load memory-mapped FAISS index."""
        ...
        if not faiss_index_file.exists():
            raise FileNotFoundError(
                ...
                f"Run: python scripts/index_slot_based.py --index {self.index_path} --build-faiss"
            )
        ...
        if hasattr(self.faiss_index, 'nprobe'):
            ...
            logger.info(f"  FAISS index: IVF with nprobe={self.faiss_index.nprobe}")
    ...
    def search(
        ...
        Stage 0: FAISS pre-filtering (4.2M → 500 docs)
        ...
        logger.debug(f"Stage 0: FAISS pre-filtering ({self.faiss_index.ntotal:,} docs → {faiss_top_n} candidates)")
        ...
        logger.debug(f"Stage 1: Mmap slot reranking ({faiss_top_n} → {slot_top_n} candidates)")

        slot_results = []
        for i, doc_id in enumerate(faiss_indices):
            ...
            combined_score = slot_weight * slot_score + full_weight * faiss_score
