...
Downloads PDF and extracts text for corpus integration.

Author: Kálmán Kalocsay, Gaston Waringhien
Edition: 5th (1985)
License: Public domain (verified)
Source: http://luisguillermo.com/PAG/
"""
...
# Constants
PAG_PDF_URL = "http://luisguillermo.com/PAG/Plena_Analiza_Gramatiko_(K._Kalocsay,_G._Waringhien).pdf"
PAG_PDF_BACKUP = "https://docs.google.com/file/d/0BwitPj8ueoxNeElRU1dydmRWUm8/edit"

def download_pdf(url: str, output_path: Path) -> bool:
    """Download PAG PDF from URL."""
    logger.info(f"Downloading PAG PDF from {url}")
...
def extract_text_from_pdf(pdf_path: Path, output_text_path: Path) -> bool:
    """Extract text from PAG PDF using PyPDF2."""
...
def clean_pag_text(raw_text: str) -> str:
    """Clean extracted text and fix OCR errors."""
    ...
    # Fix common OCR errors for Esperanto diacritics
    # (Only if we detect they're missing - preserve good text)
...
def validate_text_quality(text: str) -> dict:
    ...
    quality = {
        ...
        'sample': sample,
        'has_esperanto_chars': len(found_diacritics) >= 4,  # At least 4 different diacritics
    }
    ...
    if not quality['has_esperanto_chars']:
        logger.warning("WARNING: Few or no Esperanto diacritics found - may need OCR")

    return quality


def acquire_pag(output_dir: Path, skip_download: bool = False):
    """Main acquisition workflow."""
    logger.info("=" * 70)
    logger.info("ACQUIRE PAG (Plena Analiza Gramatiko)")
    logger.info("=" * 70)

    # Paths
    pdf_path = output_dir / 'pag.pdf'
    raw_text_path = output_dir / 'pag_raw.txt'
    cleaned_text_path = output_dir / 'pag.txt'

    # Step 1: Download PDF
    if not skip_download or not pdf_path.exists():
        success = download_pdf(PAG_PDF_URL, pdf_path)
        if not success:
    ...
    # Step 3: Clean text
    cleaned_text = clean_pag_text(raw_text)
    ...
    logger.info("1. Review pag.txt for quality (check for Esperanto diacritics)")
    logger.info("2. If quality is good, proceed to sentence extraction")
    logger.info("3. If diacritics are missing, may need OCR with Tesseract")

    return True


def main():
    parser = argparse.ArgumentParser(description='Acquire PAG (Plena Analiza Gramatiko)')
    ...
    try:
        success = acquire_pag(args.output_dir, args.skip_download)
