...
LOG_FILE="logs/classification_fast_$(date +%Y%m%d_%H%M%S).log"

echo "=========================================="
echo "Klareco Root Classification (FAST MODE)"
echo "=========================================="
echo ""
...
echo ""
echo "OPTIMIZATIONS:"
echo "  - Batched UPDATE queries (100 nodes per query)"
echo "  - Larger fetch batches (50K nodes at a time)"
echo "  - Uses UNWIND for bulk updates"
echo ""
echo "Classification:"
...
echo ""
echo "Estimated time: ~15-20 minutes (5-10x faster than standard)"
echo ""
...
# Run fast classification with logging
# -u: unbuffered Python output
# stdbuf -oL -eL: line-buffered output for tee
stdbuf -oL -eL python -u scripts/classify_roots_fast.py \
...
if [ ${PIPESTATUS[0]} -eq 0 ]; then
    echo ""
    echo "=========================================="
    echo "✓ Fast classification completed!"
    echo "=========================================="
    echo ""
    ...
    echo "  ./scripts/classify_roots_fast.sh --resume"
    exit 1
fi
