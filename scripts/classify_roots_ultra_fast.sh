...
LOG_FILE="logs/classification_ultra_fast_$(date +%Y%m%d_%H%M%S).log"

echo "=========================================="
echo "Klareco Root Classification (ULTRA FAST)"
echo "=========================================="
echo ""
...
echo "  ✓ Fetch all 1.2M radiko strings in ONE query"
echo "  ✓ Eliminates SKIP/LIMIT table scan overhead"
echo "  ✓ Transaction batching (1000 UPDATEs per transaction)"
echo "  ✓ Memory-safe: Only strings in RAM (~50-100MB)"
echo ""
echo "Classification:"
...
echo ""
echo "Estimated time: ~5-10 minutes (10-20x faster!)"
echo ""
...
# Run ultra fast classification with logging
# -u: unbuffered Python output
# stdbuf -oL -eL: line-buffered output for tee
stdbuf -oL -eL python -u scripts/classify_roots_ultra_fast.py \
...
if [ ${PIPESTATUS[0]} -eq 0 ]; then
    echo ""
    echo "=========================================="
    echo "✓ ULTRA FAST classification completed!"
    echo "=========================================="
    echo ""
    ...
    echo "  ./scripts/classify_roots_ultra_fast.sh --resume"
    exit 1
fi
