...
echo "  - PAG (needs OCR first - run ./scripts/ocr_pag.sh)"
echo ""
...
if [ -f "data/raw/eo/pag/pag_ocr.txt" ]; then
    ...
    echo "  ⚠ PAG skipped - needs OCR (run ./scripts/ocr_pag.sh)"
fi
...
if [ -f "data/raw/eo/proverbaro/proverbaro.txt" ]; then
    ...
    echo "  ⚠ Proverbaro skipped - not acquired yet (run ./scripts/acquire_proverbaro.sh)"
fi
...
echo "Next step: ./scripts/extract_all_tier0.sh"
