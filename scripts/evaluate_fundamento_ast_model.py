#!/usr/bin/env python3
"""
Evaluate Fundamento AST-aware root embeddings.

Adapted to work with skip-gram checkpoint format.

Usage:
    python scripts/evaluate_fundamento_ast_model.py \
        --model models/root_embeddings_fundamento_ast/root_embeddings_best.pt
"""
...
def load_model_and_vocab(model_path: Path):
    """Load skip-gram checkpoint and extract embeddings."""
    checkpoint = torch.load(model_path, map_location='cpu')

    # Skip-gram checkpoint format
    embeddings = checkpoint['embeddings']
    root_to_idx = checkpoint['root_to_idx']
    idx_to_root = {str(idx): root for root, idx in root_to_idx.items()}
...
def test_antonyms(embeddings: torch.Tensor, root_to_idx: Dict[str, int]) -> Dict:
    ...
    result = {
        ...
        'examples': [(p, n, float(s)) for p, n, s in examples]
    }
...
def test_semantic_clusters(
    ...
    for cluster_name, roots in test_clusters.items():
        ...
        if len(valid_roots) < 3:
            logger.warning(f"  {cluster_name}: too few roots in vocabulary ({len(valid_roots)})")
            continue
...
def test_nearest_neighbors(
    ...
    root_to_idx: Dict[str, int],
    idx_to_root: Dict[int, str],
    test_roots: List[str] = ['hund', 'am', 'bon', 'ruĝ', 'grand']
) -> Dict:
    """
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Fundamento AST embeddings")
    ...
    if args.output:
        ...
        with open(args.output, 'w') as f:
            # Convert tensors/floats for JSON serialization
