...
#
# Usage:
#   ./scripts/extract/extract_gutenberg.sh
...
# Show discovered books using Python (dynamic discovery)
echo -e "${YELLOW}→${NC} Discovering books..."
python scripts/extract/extract_gutenberg.py --list-only 2>/dev/null | tail -1
echo ""
...
# Run extraction
python scripts/extract/extract_gutenberg.py \
...
if [ $? -eq 0 ]; then
    ...
    echo -e "${YELLOW}→${NC} Command: ./scripts/parse/parse_corpus.sh"
else
    echo ""
