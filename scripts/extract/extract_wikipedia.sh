...
#
# Usage:
#   ./scripts/extract/extract_wikipedia.sh           # Resume from checkpoint (if exists)
#   ./scripts/extract/extract_wikipedia.sh --fresh   # Start fresh (delete checkpoint and output)
#   ./scripts/extract/extract_wikipedia.sh &         # Run in background
...
if [ -f "$CHECKPOINT" ] && [ -z "$FRESH_FLAG" ]; then
    ...
    echo -e "${YELLOW}→${NC} To start fresh, run: ./scripts/extract/extract_wikipedia.sh --fresh"
    echo ""
fi
...
# Run extraction
python scripts/extract/extract_wikipedia.py \
    --xml "$WIKI_DUMP" \
...
if [ $? -eq 0 ]; then
    ...
    echo -e "${YELLOW}→${NC} Command: ./scripts/extract/extract_gutenberg.sh"
else
    echo ""
    echo -e "${RED}════════════════════════════════════════════════════════════${NC}"
