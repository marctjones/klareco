...
log "  Script: ./scripts/index/corpus_to_csv_v2.1.sh"
log "  Input: data/corpus/unified_corpus.jsonl (1.6GB, 5.39M sentences)"
log "  Output: data/csv_export_v2.1_full/"
echo ""

STEP1_START=$(date +%s)
./scripts/index/corpus_to_csv_v2.1.sh 2>&1 | tee -a "$MASTER_LOG"
...
log "  Script: ./scripts/index/load_csv_to_kuzu_v2.1.sh --fresh"
log "  Input: data/csv_export_v2.1_full/"
log "  Output: data/indexes/v2.1_kuzu_index_full/"
echo ""

STEP2_START=$(date +%s)
./scripts/index/load_csv_to_kuzu_v2.1.sh --fresh 2>&1 | tee -a "$MASTER_LOG"
check_exit ${PIPESTATUS[0]}
STEP2_END=$(date +%s)
...
log "  Script: scripts/index/extend_kuzu_schema_semantic_ontology.py"
...
log "  Script: ./scripts/index/load_revo_v2.1.sh"
...
STEP4_START=$(date +%s)
./scripts/index/load_revo_v2.1.sh 2>&1 | tee -a "$MASTER_LOG"
check_exit ${PIPESTATUS[0]}
STEP4_END=$(date +%s)
