...
#
# Usage:
#   ./scripts/post_reparse_pipeline.sh
#
set -e
set -o pipefail
...
if [ -f "data/raw/eo/dictionaries/revo/revo_relations.json" ] || [ -f "data/revo/revo_semantic_relations.json" ]; then
    python scripts/load_revo_to_kuzu.py 2>&1 | tee -a "$MASTER_LOG" || log "  ReVo load failed — continuing"
else
...
python scripts/build_whoosh_index.py 2>&1 | tee -a "$MASTER_LOG" || log "  Whoosh build failed/already current"

# Step 4: Rebuild synthetic test set with new dict coverage
log "STEP 4/5: Rebuilding synthetic WHO test set..."
python scripts/build_synthetic_who_test_set.py \
    --target-size 200 \
...
python -u scripts/evaluate_extractive_qa.py \
